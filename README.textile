...
Bees uses boto to communicate with EC2 and thus supports all the methods of vstoring credentials that it does.  These include: declaring environment variables, machine-global configuration files, and per-user configuration files. You can read more about these options on "boto's configuration page":http://code.google.com/p/boto/wiki/BotoConfig
...
This spins up 4 servers in security group 'public' using the EC2 key 'frakkingtoasters', which is expected to reside in ~/.ssh/.
